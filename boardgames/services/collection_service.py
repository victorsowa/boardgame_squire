...
def get_user_games_from_boardgamegeek(username):
    ...
    try:
        ...
        pass
    try:
        if collection_et.text.strip() == 'Your request for this collection has been accepted and will be processed.  Please try again later for access.':
            return 'waiting'
    except AttributeError:
        pass
...
def add_new_users_collection_to_db(username):
    ...
    if game_ids_not_already_in_database is not None:
        general_game_data_for_user_games = get_general_game_data_from_boardgamegeek(game_ids_not_already_in_database)
